...
- [x] 塗鴉模式雷雕情境：SVG 丟進它的 workflow 反向驗證（2026-07-11
      實機，樣本＝name_tag 布章「永日」80×40mm，含 5cc 線寬修正）：
      1. **SVG 直餵意外可用**——宣稱只收 PNG/JPG/WebP，實測瀏覽器
         代為光柵化（@96dpi，302×151px ↔ 80×40mm，mm 契約被正確
         尊重）。但 0.3mm 單線外框在 96dpi 二值化下丟失；字形因
         雙線輪廓＋反鋸齒疊加夠黑而倖存 → 可用但不建議
      2. **300dpi PNG 完整 round-trip**——外框＋字形全保留
         （945×472px、30 路徑、1325 節點），「電子圖檔最佳化」
         預設向量化乾淨。建議工作流：我們的 SVG → cairosvg
         300dpi PNG → VectorLine → 其 SVG/DXF
      3. 5cc 髮絲線修正在第三方光柵器實戰過關（修正前字形會
         跟外框一樣消失）
