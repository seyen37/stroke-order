...
## 98. 引用自己先前的量測前，先驗量測條件是否仍成立（2026-08-20 B 輪新增）

R1b 量出真輪廓 ±δ 的「安全區 ±20 EM ≈ 筆寬 ±9%」（國/歡/明、50 mm 字框）。
B 輪把它當夾限依據動工，中途再量 noto_hei（粗黑體）：**+20 就讓「歡」黏合
（3→1 件）、「國」孔 2→5**——等於楷書在 +40 才發生的事。安全區沒有錯，
錯的是**引用**：R1b 的量測條件是楷書級筆寬，粗筆畫字源間隙小、黏合門檻
等比提前。

### 98.1 §93 管「量的當下」，這條管「引用的當下」

§93 說中間量也要當結論看待、指標要拿已知答案自驗——那是量測**產生**時的
紀律。這條補的是量測**消費**時的紀律：一個數字被再次引用時，要問「當初是
在什麼條件下量的？字源／尺度／環境／版本，哪個變了？」。變了就重量，不變
才引用。自己量的數字最危險——因為信任是現成的，沒有「查證外部來源」的
警覺（§91 對外部授權有這條防線，對自己的數字反而沒有）。

### 98.2 處置的形狀

B 輪的處置可以當模板：(1) **數字保留原效力範圍**——夾限仍鎖 ±20，因為
「超出連楷書都壞」仍成立；(2) **措辭降級**——「安全區」改「楷書級實測
範圍」，不許諾量測沒涵蓋的事；(3) **新條件的實測值釘成已知答案樁**——
noto_hei 的黏合行為進測試，防日後有人把夾限當跨字源保證放寬；(4) 後果
交給**執行期可觀測的回報**（件數/孔數標頭），不是文件裡的一句警語。

---

## 99. 緩議是一等結案：翻案條件要可觀測、路徑要鋪到能直接動工（2026-08-20 R2 歸納）

架上項目有三種合法終態：**出貨、緩議、撤案**——「做完」不是唯一的完成。
但緩議要成為結案而不是拖延，得滿足兩個條件：

### 99.1 翻案條件要可觀測，最好儀器化

T3（插圖）緩議時留了量測數據供翻案——夠好，但翻案條件（「上游覆蓋率
改善」）要靠人記得去查。R2（GlyphWiki）多走一步：翻案條件是「使用者
真的撞到筆順鏈缺字」，而這件事可以**儀器化**——筆順鏈 404 時記一筆
「缺字請求」計數，需求證據自己長出來，不用人猜也不用人盯。

「等需求證據」不是被動等。緩議的當下就該問：**這個條件發生時，系統會
自己告訴我們嗎？** 會＝真結案；不會＝加一個最便宜的感測器，或誠實承認
這是撤案。

### 99.2 路徑要鋪到能直接動工

spike 的產出不只是「做／不做」的結論，還有**已驗證的動工路徑**——R2 的
ADR 寫明五步（dump 管道→bundle 慣例→解析器落點→誠實標注→授權聲明），
翻案那天不必重做 spike。判準：一個沒讀過本輪脈絡的人，拿著 ADR 能不能
直接開工？能，緩議才算把 spike 的錢花完。

### 99.3 邊界

緩議不是預設終態——它適用於「價值真實但需求未證實」的項目。價值本身
被否證的（如 W4 前身的假年級軸），直接撤案，不留翻案路徑：鋪路也是成本。
...
- 決策紀錄：
  ...
  - [`2026-08-20_x1_steps_strip.md`](decisions/2026-08-20_x1_steps_strip.md)（X1 筆順分解圖三決策 QODA：新端點批次出圖不動 grid／已寫淡灰＋當前濃黑（灰階單調，刻意不沿用 gif 紅色 highlight）＋字形路徑序號白底護墊／envelope mode=None 不進分享庫12 模式數量鎖；附比例寬數字套全形步進的產圖才見之錯——§96 第四次生效）
